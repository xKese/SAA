...
import { AssetAllocation, getChartColor, formatCurrency, formatPercentage } from "@/types/analysis";
...
export function AssetAllocationChart({ data, totalValue }: AssetAllocationChartProps) {
  const [expandedRows, setExpandedRows] = useState<Set<string>>(new Set());

  const toggleRowExpansion = (category: string) => {
    const newExpandedRows = new Set(expandedRows);
    if (newExpandedRows.has(category)) {
      newExpandedRows.delete(category);
    } else {
      newExpandedRows.add(category);
    }
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-center h-64 text-gray-500">
            Keine Asset-Allokations-Daten verfügbar
          </div>
        </CardContent>
      </Card>
    );
            ...
            <tbody>
              {data.map((item, index) => (
                <>
                    ...
                    <td className="py-2">
                      {item.assignedPositions && item.assignedPositions.length > 0 && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => toggleRowExpansion(item.category)}
                  ...
                  {item.assignedPositions && item.assignedPositions.length > 0 && expandedRows.has(item.category) && (
